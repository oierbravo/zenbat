...
/** Possible header names for component code and unit price in productos/componentes XLSX. */
const ID_FIELD_ALIASES = ['codigo', 'Codigo', 'CODIGO', 'Código'];
const PRICE_FIELD_ALIASES = ['precioUnit', 'precioUnitario', 'PrecioUnitario', 'Precio Unitario', 'PrecioUnit', 'precio_unitario'];
...
/**
 * Load productos.xlsx via zenbat-xlsx and return a Map: codigo (number-only) -> precioUnit.
 * Uses readComponentesXlsx; accepts sheet "componentes" or first sheet. Tries common column names (codigo/Codigo, precioUnit/PrecioUnitario/Precio Unitario).
 * @param {string} filePath - Path to productos.xlsx
 * @returns {Map<string, number|string>}
 */
export function loadProductosPriceMap(filePath) {
  const map = new Map();
  if (!filePath || !fs.existsSync(filePath)) return map;
  try {
    const { raw } = readComponentesXlsx(filePath);
    for (const row of raw) {
      if (!row) continue;
      const codigoRaw = getRowValue(row, ID_FIELD_ALIASES) ?? getRowValue(row, [DEFAULT_ID_FIELD]);
      const codigo = codigoRaw != null ? String(codigoRaw).trim() : '';
      const precioUnit = getRowValue(row, PRICE_FIELD_ALIASES) ?? getRowValue(row, [DEFAULT_PRICE_FIELD]);
      if (codigo && precioUnit != null && precioUnit !== '') {
        const key = armarioIdNumberOnly(codigo) || codigo;
        const num = typeof precioUnit === 'number' ? precioUnit : parseFloat(String(precioUnit));
        map.set(key, Number.isNaN(num) ? precioUnit : num);
      }
    }
  } catch (err) {
    console.warn('Could not load productos price map:', err.message);
  }
  return map;
}

/**
 * Enrich each component with PrecioUnitario (from price map) and PrecioTotal (quantity × price).
 * @param {object[]} components
 * @param {Map<string, number|string>} priceMap
 */
function enrichComponentsWithPrices(components, priceMap) {
  for (const c of components) {
    const codigo = c.Codigo ?? c.componentId ?? '';
    const price = priceMap && priceMap.size > 0 && codigo ? (priceMap.get(codigo) ?? priceMap.get(armarioIdNumberOnly(codigo))) : null;
    const qty = parseFloat(c.quantity ?? c.Cantidad ?? 0) || 0;
    c.PrecioUnitario = price != null ? price : '';
    c.PrecioTotal = price != null && !Number.isNaN(qty) ? (typeof price === 'number' ? price * qty : (parseFloat(String(price)) || 0) * qty) : '';
  }
}

/** Ensure every component has PrecioUnitario and PrecioTotal (for consistent JSON/CSV/MD/XLSX/HTML). */
function ensurePriceFields(components) {
  for (const c of components) {
    if (!Object.hasOwn(c, 'PrecioUnitario')) c.PrecioUnitario = '';
    if (!Object.hasOwn(c, 'PrecioTotal')) c.PrecioTotal = '';
  }
}
...
const PARSED_MD_HEADERS = ['Codigo', 'Denominacion', 'Unidad', 'Cantidad', 'PrecioUnitario', 'PrecioTotal', 'Ficticio'];
...
function writeHtml(outPath, head, components, headerRow, tableHeaderLines = []) {
  ...
  const componentLabels = { componentIdRaw: 'Ref.', componentId: 'Código', description: 'Denominación', unitMeasure: 'Unidad', quantity: 'Cantidad', PrecioUnitario: 'Precio unit.', PrecioTotal: 'Precio total', ficticio: 'Ficticio' };
    ...
    for (const { unit, components: group } of byUnit) {
      ...
      for (const c of group) {
        ...
        for (const [key, label] of Object.entries(componentLabels)) {
          ...
          } else if (key === 'PrecioUnitario' || key === 'PrecioTotal') {
            const val = c[key];
            parts.push(`<dt>${escapeHtml(label)}</dt><dd>${val != null && val !== '' ? escapeHtml(String(val)) : '—'}</dd>`);
 ...
 * @param {object} [options] - { xlsx: boolean, columnMapPath: string, productosPath?: string, productosPriceMap?: Map }
 * @returns {Promise<{ head: any, components: object[] }>}
 */
export async function extractArmarioFromPdf(inputPath, outputPath, options = {}) {
  const { xlsx = false, columnMapPath, lines = false, html = false, productosPath, productosPriceMap } = options;
  ...
  const priceMap = productosPriceMap ?? (productosPath ? loadProductosPriceMap(productosPath) : null);
  enrichComponentsWithPrices(components, priceMap);
  ensurePriceFields(components);

  let effectiveHeaderRow = headerRow || [];
  if (effectiveHeaderRow.length > 0 && !effectiveHeaderRow.includes('PrecioUnitario')) {
    effectiveHeaderRow = [...effectiveHeaderRow, 'PrecioUnitario', 'PrecioTotal'];
  }

  writeJson(basename + '.json', head, components);
  writeCsv(basename + '.csv', components, effectiveHeaderRow);
  writeMarkdown(basename + '.md', head, components, effectiveHeaderRow);
  if (xlsx) writeArmarioXlsx(basename + '.xlsx', components.map((row) => toArmarioRow(row)));
  if (html) writeHtml(basename + '.html', head, components, effectiveHeaderRow, tableHeaderLines);
