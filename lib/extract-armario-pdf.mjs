 ...
 * Uses readComponentesXlsx; sheet must be named "componentes" and have codigo/precioUnit columns.
 * @param {string} filePath - Path to productos.xlsx
 * @returns {Map<string, number|string>}
 */
function loadProductosPriceMap(filePath) {
  const map = new Map();
  if (!filePath || !fs.existsSync(filePath)) return map;
  try {
    const { raw } = readComponentesXlsx(filePath);
    const idField = DEFAULT_ID_FIELD;
    const priceField = DEFAULT_PRICE_FIELD;
    for (const row of raw) {
      if (!row) continue;
      const codigo = row[idField] != null ? String(row[idField]).trim() : '';
      const precioUnit = row[priceField];
 ...
 * @param {object} [options] - { xlsx: boolean, columnMapPath: string, productosPath?: string }
 * @returns {Promise<{ head: any, components: object[] }>}
 */
export async function extractArmarioFromPdf(inputPath, outputPath, options = {}) {
  const { xlsx = false, columnMapPath, lines = false, html = false, productosPath } = options;
  ...
  const priceMap = productosPath ? loadProductosPriceMap(productosPath) : null;
